...
var _deck = deck,
    DeckGL = _deck.DeckGL,
    MVTLayer = _deck.MVTLayer;
var shouldUpdate = 1;
var viz;
var baseURL = window.location.href.replace('/example-data-loading/', '');
console.log('base url', baseURL);
var gemeindenURL = "".concat(baseURL, "/tiles/gemeinden-z7-ids/{z}/{x}/{y}.pbf");
var gemeindenData = [gemeindenURL];
var kreiseURL = "".concat(baseURL, "/tiles/kreise-z8-Z6-filtered-ai/{z}/{x}/{y}.pbf");
var kreiseData = [kreiseURL];
var laenderURL = "".concat(baseURL, "/tiles/laender-z6-ids/{z}/{x}/{y}.pbf");
var laenderData = [laenderURL];
var fakeData = {
  laender: new Array(34).fill(0).map(function (_, i) {
    return Math.random();
  }),
  gemeinden: new Array(11431).fill(0).map(function (_, i) {
    return Math.random();
  }),
  kreise: new Array(432).fill(0).map(function (_, i) {
    return Math.random();
  })
};
var numDates = 100;
var d = new Date();
var currentDate = 0;
d.setDate(-numDates + currentDate);
var dateStr = d.toLocaleDateString('en-CA'); //date.format(d, 'YYYY-MM-DD')

console.log('current date is ', dateStr);

function render() {
  var gemeinden = new MVTLayer({
    id: 'gemeindengeo',
    data: gemeindenData,
    pickable: true,
    getFillColor: function getFillColor(obj) {
      var j = fakeData.gemeinden[obj.id];
...
setInterval(function () {
  ...
  fakeData.gemeinden = fakeData.gemeinden.map(function (_, i) {
    return Math.random();
  ...
  render();
  shouldUpdate++;
}, 2000); //   const tileURL = `${window.location.origin}/gemeinden-z7-uncompressed/{z}/{x}/{y}.pbf`
// const DATA = [
//     tileURL
